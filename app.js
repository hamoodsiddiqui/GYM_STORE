...
app.get('/customer/products',checkCustomer, (req, res) => {
...
app.get('/merchandise',checkAdmin, (req, res) => {
  ...
  db.query(selectMerchandiseQuery, (err, merchandise) => {
    if (err) {
      ...
      res.status(500).json({ error: 'An error occurred while fetching merchandise' });
    } else {
      // Now, run another query to fetch products with product_type = "Merchandise"
      const selectProductsQuery = 'SELECT * FROM Products WHERE product_type = ?';
      db.query(selectProductsQuery, ['Merchandise'], (err, products) => {
        if (err) {
          ...
          const combinedData = merchandise.map(merchandiseItem => {
            const matchingProduct = products.find(product => product.product_id === merchandiseItem.product_id);
      ...
      db.query(selectProductsQuery, ['Supplements'], (err, products) => {
        if (err) {
          ...
          const combinedData = supplements.map(supplement => {
            const matchingProduct = products.find(product => product.product_id === supplement.product_id);
...
app.get('/gymequipment',checkAdmin, (req, res) => {
  ...
  db.query(selectGymEquipmentQuery, (err, gymEquipment) => {
    if (err) {
      ...
      res.status(500).json({ error: 'An error occurred while fetching gym equipment' });
    } else {
      ...
      db.query(selectProductsQuery, ['Gym Equipment'], (err, products) => {
        if (err) {
          ...
          const combinedData = gymEquipment.map(equipment => {
            const matchingProduct = products.find(product => product.product_id === equipment.product_id);
...
app.get('/supplements/details/:productId', (req, res) => {
  ...
  const selectSupplementQuery = 'SELECT * FROM Supplements WHERE product_id = ?';
  ...
  db.query(selectSupplementQuery, [productId], (err, supplementData) => {
    if (err) {
      ...
      return res.status(500).json({ error: 'An error occurred while fetching supplement data' });
    }
    ...
    db.query(selectProductQuery, [productId], (err, productData) => {
      ...
      const combinedData = supplementData.map(supplement => {
        const matchingProduct = productData.find(product => product.product_id === supplement.product_id);
        return { ...supplement, ...matchingProduct };
      });
...
app.get('/gymequipment/details/:productId', (req, res) => {
  ...
  // Combine data from both tables as needed
  const selectGymEquipmentQuery = 'SELECT * FROM GymEquipment WHERE product_id = ?';
  const selectProductQuery = 'SELECT * FROM Products WHERE product_id = ?';

  db.query(selectGymEquipmentQuery, [productId], (err, gymEquipmentData) => {
    if (err) {
      console.error('Error fetching gym equipment data:', err);
      return res.status(500).json({ error: 'An error occurred while fetching gym equipment data' });
    }
    ...
    db.query(selectProductQuery, [productId], (err, productData) => {
      ...
      const combinedData = gymEquipmentData.map(equipment => {
        const matchingProduct = productData.find(product => product.product_id === equipment.product_id);
        return { ...equipment, ...matchingProduct };
      });
...
app.get('/merchandise/details/:productId', (req, res) => {
  ...
  // Combine data from both tables as needed
  const selectMerchandiseQuery = 'SELECT * FROM Merchandise WHERE product_id = ?';
  const selectProductQuery = 'SELECT * FROM Products WHERE product_id = ?';

  db.query(selectMerchandiseQuery, [productId], (err, merchandiseData) => {
    if (err) {
      console.error('Error fetching merchandise data:', err);
      return res.status(500).json({ error: 'An error occurred while fetching merchandise data' });
    }
    ...
    db.query(selectProductQuery, [productId], (err, productData) => {
      ...
      const combinedData = merchandiseData.map(merchItem => {
        const matchingProduct = productData.find(product => product.product_id === merchItem.product_id);
      ...
      db.query(selectProductsQuery, ['Merchandise'], (err, products) => {
        if (err) {
          ...
          const combinedData = merchandise.map(merchItem => {
            const matchingProduct = products.find(product => product.product_id === merchItem.product_id);
            return { ...merchItem, ...matchingProduct };
          });
...
app.post('/gymequipment/add', upload.single('image'), (req, res) => {
  const { product_name, description, price, stock_quantity, brand, material, weight } = req.body;
  const product_type = "Gym Equipment";

  // Check if an image was uploaded
  const imageurl = req.file ? 'uploads/' + req.file.filename : null;

  if (product_name && description && price && stock_quantity && brand && material && weight) {
    // Create a new product
    const productQuery = 'INSERT INTO Products (product_type, product_name, brand, description, imageurl, price, stock_quantity) VALUES (?, ?, ?, ?, ?, ?, ?)';
    db.query(productQuery, [product_type, product_name, brand, description, imageurl, price, stock_quantity], (err, result) => {
      if (err) {
        ...
        const gymEquipmentQuery = 'INSERT INTO GymEquipment (product_id, material, weight) VALUES (?, ?, ?)';
        db.query(gymEquipmentQuery, [productId, material, weight], (err, result) => {
          if (err) {
            console.error('Error creating new gym equipment:', err);
            res.status(500).json({ error: 'An error occurred while creating the gym equipment' });
          } else {
...
app.post('/merchandise/add', upload.single('image'), (req, res) => {
  const { product_name, description, price, stock_quantity, brand, size, color } = req.body;
  const product_type = "Merchandise";

  // Check if an image was uploaded
  const imageurl = req.file ? 'uploads/' + req.file.filename : null;

  if (product_name && description && price && brand && stock_quantity && size && color) {
    // Create a new product
    const productQuery = 'INSERT INTO Products (product_type, product_name,brand, description, price, stock_quantity, imageurl) VALUES (?, ?,?,?, ?, ?, ?)';
    db.query(productQuery, [product_type, product_name, brand, description, price, stock_quantity, imageurl], (err, result) => {
      if (err) {
        console.error('Error creating a new product:', err);
        res.status(500).json({ error: 'An error occurred while creating the product' });
      } else {
        ...
        const merchandiseQuery = 'INSERT INTO Merchandise (product_id, size, color) VALUES (?, ?, ?)';
        db.query(merchandiseQuery, [productId, size, color], (err, result) => {
          if (err) {
            console.error('Error creating new merchandise:', err);
            res.status(500).json({ error: 'An error occurred while creating the merchandise' });
          } else {
...
app.post('/supplements/delete/:product_id', (req, res) => {
  ...
  db.query(deleteSupplementQuery, [product_id], (err, supplementResult) => {
    if (err) {
      ...
      res.status(500).json({ error: 'An error occurred while deleting the supplement' });
    } else {
      //delete the product
      ...
      db.query(deleteProductQuery, [product_id], (err, productResult) => {
        if (err) {
          ...
          console.log(`Product with ID ${product_id} and its supplement deleted successfully`);
    ...
    if (err) {
      ...
      db.query(deleteProductQuery, [product_id], (err, productResult) => {
        if (err) {
          ...
          console.log(`Product with ID ${product_id} and its merchandise deleted successfully`);
    ...
    if (err) {
      ...
      db.query(deleteProductQuery, [product_id], (err, productResult) => {
        if (err) {
          ...
          console.log(`Product with ID ${product_id} and its gym equipment deleted successfully`);
...
app.post('/gymequipment/update', upload.single('image'), (req, res) => {
  ...
  // console.log(`${product_id}`);
  const { product_name, description, price, stock_quantity, weight, brand } = req.body;
  const imageurl = req.file ? 'uploads/' + req.file.filename : null;
  console.log('Received data:', req.body);
  console.log(`Product Name: ${product_name} Description: ${description} Price: ${price} Stock : ${stock_quantity} weight : ${weight}`);

  // Create the update query for the GymEquipment table
  const updateGymEquipmentQuery = 'UPDATE GymEquipment SET weight = ? WHERE product_id = ?';
  db.query(updateGymEquipmentQuery, [weight, product_id], (err, result) => {
    if (err) {
      console.error('Error updating gym equipment:', err);
      res.status(500).json({ error: 'An error occurred while updating the gym equipment' });
    } else {
      console.log('Gym equipment updated successfully');

      // Check if product attributes are provided for update
      if (product_name && brand && description && price && stock_quantity && imageurl) {
        // Create the update query for the Products table
        const updateProductQuery = 'UPDATE Products SET product_name = ?, brand = ?, description = ?, imageurl = ?, price = ?, stock_quantity = ? WHERE product_id = ?';
        db.query(updateProductQuery, [product_name, brand, description, imageurl, price, stock_quantity, product_id], (err, result) => {
          if (err) {
...
app.post('/merchandise/update', upload.single('image'), (req, res) => {
  ...
  const imageurl = req.file ? 'uploads/' + req.file.filename : null;
  console.log('Received data:', req.body);
  console.log(`Product Name: ${product_name} Description: ${description} Price: ${price} Stock : ${stock_quantity} size : ${size},color: ${color}`);

  // Create the update query for the Merchandise table
  const updateMerchandiseQuery = 'UPDATE Merchandise SET size = ?,color=? WHERE product_id = ?';
  db.query(updateMerchandiseQuery, [size, color, product_id], (err, result) => {
    if (err) {
      console.error('Error updating merchandise:', err);
      res.status(500).json({ error: 'An error occurred while updating the merchandise' });
    }
    else if
...
app.post('/supplements/update', upload.single('image'), (req, res) => {
  ...
  const updateSupplementQuery = 'UPDATE Supplements SET weight = ?, expiry_date = ? WHERE product_id = ?';
  db.query(updateSupplementQuery, [weight, expiry_date, product_id], (err, result) => {
    if (err) {
      console.error('Error updating supplement:', err);
      res.status(500).json({ error: 'An error occurred while updating the supplement' });
    } else {
    ...
    db.query(createOrderQuery, [userId, 'Processing'], (err, orderResult) => {
      ...
      db.query(insertOrderItemsQuery, [orderItemsValues], (err) => {
        if (err) {
          ...
          return res.status(500).json({ error: 'An error occurred while inserting into OrderItems' });
        }
        ...
        cartItems.forEach(item => {
          ...
          if (remainingQuantity <= 0) {
            ...
            // If remaining quantity is greater than 0, update the quantity in Products table
            const updateProductQuery = 'UPDATE Products SET stock_quantity = ? WHERE product_id = ?';
            db.query(updateProductQuery, [remainingQuantity, item.product_id], (err) => {
              if (err) {
                console.error('Error updating stock quantity in Products table:', err);
              }
            });
          }
        });
          ...
          db.query(
            ...
            (err) => {
              if (err) {
                ...
                return res.status(500).json({ error: 'An error occurred while inserting into ShippingDetails' });
              }
      ...
      if (cartData.length === 0) {
        ...
        db.query(insertCartQuery, [req.session.userId], (err, result) => {
          if (err) {
            ...
            return res.status(500).json({ error: 'An error occurred while creating new cart' });
          }
        ...
        if (cartItemData.length === 0) {
          ...
          db.query(insertCartItemQuery, [cartId, product_id, 1], (err) => {
            if (err) {
              ...
              return res.status(500).json({ error: 'An error occurred while adding item to cart' });
            }
          ...
          const updateCartItemQuery = 'UPDATE CartItems SET quantity = quantity + 1 WHERE cart_id = ? AND product_id = ?';
          db.query(updateCartItemQuery, [cartId, productId], (err) => {
            if (err) {
              console.error('Error updating item quantity in cart:', err);
              return res.status(500).json({ error: 'An error occurred while updating item quantity in cart' });
            }
...
app.post('/merchandise/add-to-cart/:productId', (req, res) => {
  ...
  db.query(selectMerchandiseQuery, [productId], (err, merchandiseData) => {
    ...
    const { product_id, product_name, price, stock_quantity } = merchandiseData[0];
